...
<title>SzabistCarpool - View All Rides</title>

 <!-- <link rel="stylesheet" href="bootstrap/css/bootstrap.min.css"/>  -->
<link rel="stylesheet" href="transfers/css/styler.css" />
<link rel="stylesheet" href="transfers/css/theme-pink.css"
	id="template-color" />
			...
			<nav role="navigation" class="main-nav">
				<ul>
					...
					<li><a href="destinations.html" title="Destinations">Destinations</a>
						<ul>
							<li><a href="destination-single.html"
								title="Single destination">Single destination</a></li>
							<li><a href="destination-micro.html"
								title="Micro destination">Micro destination</a></li>
						</ul></li>
					<li><a href="tailor-made.html" title="Tailor made">Tailor
							made</a></li>
...
<table class="table">
		...
		<c:forEach items="${AllRides}" var="rides">
		<tr>
		<%-- <c:set var="rides" scope="session" value="${rides.sourcelocation}"></c:set> --%>
		<td><c:out value="${rides.sourcelocation}"/></td>
		<td><c:out value="${rides.destinationlocation}"/></td>
		<td><c:out value="${rides.date}"/></td>
		<td><c:out value="${rides.fair}"/></td>
		<td><c:out value="${rides.totalseat}"/></td>
		<td><c:out value="${rides.availableseat}"/></td>
		<td><c:out value="${rides.carname}"/></td>
		<td><c:out value="${rides.carmodel}"/></td>
		<td><c:out value="${rides.fkUserId}"/></td>
		<%-- <input type="hidden" name="rideID" id="rideID" value="${rides.postrideId}"> --%>
		<th><a href="bookpassenger?rideId=${rides.postrideId}"><input  type="Submit" class="btn grey small" value="Book a Ride"></a></th>
		<th><a href="ride/discussion"><input type="button" class="btn grey small" value="Contact Driver"></a></th>
		</tr></c:forEach>
...
<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
	...
	$("[data-postrideId]").click(function(){
		/* var tempId = $(this).data("postrideId"); */
		var temiID = document.getElementById('rideID').value;
		alert(temiID);
	});
	</script> -->
	<script type="text/javascript">
	function getRideId(RideId){
		alert(RideId);
	}
	</script>
	<!-- TEMPLATE STYLES -->
